...
namespace WOD.Game.Server.Feature.GuiDefinition.ViewModel
{
    public class BugReportViewModel: GuiViewModelBase<BugReportViewModel, GuiPayloadBase>
    {
        ...
        public Action OnClickSubmit() => () =>
        {
            ...
            var url = Environment.GetEnvironmentVariable("WOD_BUG_WEBHOOK_URL");

            if (string.IsNullOrWhiteSpace(url))
            {
                SendMessageToPC(Player, ColorToken.Red("ERROR: Unable to send bug report because server admin has not specified the 'WOD_BUG_WEBHOOK_HOST' and 'WOD_BUG_WEBHOOK_PATH' environment variables."));
                return;
            }
